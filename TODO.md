-> Fix the end tag detector
 -> It throws an error when it sees an invalid en tag
 ...
 -> It should check for a complete end tag instead
  ...
  -> It should not assume there is a `>` and inore it, but instead read from the `</` and up to the `>` and check what's between
